...
This repository hosts the Homebrew cask that installs the `qilin` CLI on macOS. The cask is generated and published automatically by [GoReleaser](https://goreleaser.com/) from the upstream [`dev-creations/qilin`](https://github.com/dev-creations/qilin) repository on every tagged release — please don't edit `Casks/qilin.rb` by hand; open issues and PRs against the upstream repo instead.
